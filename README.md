...
- Сделать рефакторинг массового добавления товаров в корзину, сейчас они добавляются один за другим, а надо чтобы по каждому товару в один момент шли запросы к серверу (`Promise.all`).
