...
- Теперь в хуке `useKeyPressed()` [src/hooks/use-key-pressed](src/hooks/use-key-pressed.ts) работают подсказки для названий клавиш. При этом, благодаря дженерику, также можно ввести любую клавишу которой ещё нет в типе. Cам тип можно дополнять по мере необходимости, чтобы сразу все клавиши не перечислять. Вобще, эти подсказки оказались очень лаконичной и выразительной альтернативой перечислениям (в некоторых случаях), вместо чего-то вроде `KeyboardKeys.Enter` мы пишем просто `"Enter"`.

![Подсказка названия клавиши](img/useKeyPressed-1.png)

- Пока идёт загрузка списка стран (она начинается при клике на выпадающее меню), то отображается спиннер.

![Спиннер, пока идёт загрузка списка стран](img/countries-4.png)

- Меню может выпадать наверх. Учитывается ресайз окна и завершение скролла, но можно настроить без слушания событий, чтобы оно выпадало наверх только при обновлении компонента (например, в момент когда на него кликнули чтобы открыть).

- Для слишком длинных названий реализовал белое "затенение", чтобы название как бы пропадало. Когда пользователь подводит мышку к стране с таким названием в списке, то ячейка расширяется и название отображается полностью в две строки. Реализовать затенение в списке оказалось гораздо сложнее, чем мне показалось в начале. Проблема заключается в том, что когда пользователь подводит мышку, фон становится сине-фиолетовым, при этом белое затенение нужно убрать. Сначала я реализовал это на чистом `css`, добавив блок с затенением через `::after`, а затем меняя `z-index` при подведении мышки. Но оказалось, что `z-index` меняется медленно, затенение пропадает не сразу и видно мигание. Тогда я реализовал это через реакт, при этом совсем от этого эффекта избавиться не удалось (хотя с реактом это работает немного быстрее), но теперь затенение отображается только у длинных названий, а так как их мало, то эффект практически не заметен. И ещё нужно было учесть, что когда итем выделен, но к нему не подведён курсор, то градиент должен быть не белым, а в цвет выделения.

![Выбрана страна с длинным названием](img/countries-1.png)

![Отображение страны с длинным названием в списке](img/countries-2.png)

![При подведение курсора полное длинное название в списке](img/countries-3.png)

![Выпадение меню наверх](img/countries-5.png)

- Переиспользовал компонент `List`, переопределив его стили. Чтобы убрать пунктирные разделители между элементами списка просто использовал более специфичный селектор:

```css
.SelectCustomView-menu > .List > .List-item {
    border-bottom: none;
}
```

- Для решения проблемы `Кот д&#039;Ивуар` -> `Кот д'Ивуар` использовал метод `unescape` библиотеки `he`.

- Проблема наложения бордеров выпадающего списка [src/components/select-custom-view/style.css](src/components/select-custom-view/style.css). После добавления бордера, возникла проблема наложения бордеров выпадающего меню и самого поля, если мы делаем нижний бордер поля прозрачным и верхний бордер выпадающего меню тоже прозрачный (чтобы они выглядели как единое целое). Из-за того, что оба элемента имеют белый фон, а бордер срезается треугольником (под которым остается белый фон), возникает проблема наложения этого белого треугольника на бордер, которая при более толстом бордере более заметна и выглядит так:

![Проблема наложения бордеров](img/borderProblem-1.png)

В качестве решения использовал `background-clip: padding-box` и сдвиг. Альтернативно можно было бы сделать враппер c `overflow: hidden` который ровненько отрезал бы бордер с фоном. Был ещё один вариант: добавление дополнительного блока через `css` используя `::after`, и этот блок уже подвинуть так, чтобы он перекрыл промежуток между границами находясь позади. Но у этого способа оказалась неожиданная проблема: при изменении масштаба страницы возникли артефакты несовпадения бордеров.
