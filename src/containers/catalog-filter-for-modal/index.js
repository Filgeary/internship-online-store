...
function CatalogFilterForModal() {
  ...
  const callbacks = {
    // Сортировка
    onSort: useCallback(sort => store.actions.catalog_modal.setParams({sort}), [store]),
    // Поиск
    onSearch: useCallback(query => store.actions.catalog_modal.setParams({query, page: 1}), [store]),
    // Сброс
    onReset: useCallback(() => store.actions.catalog_modal.resetParams(), [store]),
    // Фильтр по категории
    onCategory: useCallback(category => store.actions.catalog_modal.setParams({category, page: 1}), [store]),
  };
