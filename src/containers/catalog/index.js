...
function Catalog({ children, ...props }) {
  ...
  const callbacks = {
    ...
    // Пагинация
    onPaginate: useCallback(page => store.actions[props.stateName].setParams({page}, false), [store]),
    ...
    // Сортировка
    onSort: useCallback(sort => store.actions[props.stateName].setParams({sort}, false), [store]),
    // Поиск
    onSearch: useCallback(query => store.actions[props.stateName].setParams({query, page: 1}, false), [store]),
    // Сброс
    onReset: useCallback(() => store.actions[props.stateName].resetParams(), [store]),
    // Фильтр по категории
    onCategory: useCallback(category => store.actions[props.stateName].setParams({category, page: 1}, false), [store]),
  };
