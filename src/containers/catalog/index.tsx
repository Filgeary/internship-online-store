...
function Catalog({ children, ...props }: CatalogProps) {
  ...
  const callbacks = {
    ...
    addToBasket: useCallback((_id: string | number) => store.actions.basket.addToBasket(_id), [store]),
    // Пагинация
    onPaginate: useCallback((page: number) => store.actions[props.stateName].setParams({page}, false), [store]),
    // генератор ссылки для пагинатора
    makePaginatorLink: useCallback((page: string) => {
    ...
    openModalOfCount: useCallback((item: TItem) => {
    ...
    // Сортировка
    onSort: useCallback((sort: string) => store.actions[props.stateName].setParams({sort}, false), [store]),
    // Поиск
    onSearch: useCallback((query: string) => store.actions[props.stateName].setParams({query, page: 1}, false), [store]),
    // Сброс
    onReset: useCallback(() => store.actions[props.stateName].resetParams(), [store]),
    // Фильтр по категории
    onCategory: useCallback((category: string) => store.actions[props.stateName].setParams({category, page: 1}, false), [store]),
  };
