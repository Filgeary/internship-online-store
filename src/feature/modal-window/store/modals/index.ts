...
import {TModal, TModalName} from "@src/ww-old-store-postponed-modals/modals/types";
...
class ModalsState extends StoreModule<TModalState> {
   ...
   * @example
   * handler () => {
   *  const result = await ww-old-store-postponed-modals.actions.modals.open(name, data)
