...
class SessionState extends StoreModule<ISessionState> {
   ...
   */
  async signIn(data: {login: string, password: string}, onSuccess?: Function): Promise<void> {
    this.setState(this.initState(), 'Авторизация');
    try {
      const res = await this.services.api.request<{user: IUser, token?: string}>({
  ...
  async remind(): Promise<void> {
    ...
    if (token) {
      ...
      const res = await this.services.api.request<RemindResponse>({url: '/api/v1/users/self'});
