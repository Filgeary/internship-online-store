...
import { IItemManyProductsProps } from './types';
import './style.css';

function ItemManyProducts(props: IItemManyProductsProps) {
  ...
  const callbacks = {
    ...
    onSelect: (e: React.MouseEvent) => props.onSelectProduct(props.item),
    // Меняем количество на месте
    onEditPcs: (value: string) => props.onChangePcs(props.item._id, value),
    // Скрыть/показать кнопку
    setShow: (val: boolean) => (e: React.MouseEvent) => {
