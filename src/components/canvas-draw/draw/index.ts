import {
  ...
  ScrollParams,
  ZoomParams,
} from "@src/components/canvas-draw/draw/types";
...
class Draw {
  ...
  constructor(count: number) {
    if (count > 0 && this.count !== count) {
      for (let i = 0; i < count; i++) {
        let tempRad = 10 + Math.floor(Math.random() * 25);
        let tempX = Math.floor(Math.random() * (984 - tempRad));
        let tempY = Math.floor(Math.random() * (550 - tempRad));
        ...
        this.elements.push(new Square(tempX, tempY, tempColor));
      }
      this.count = count;
    }
  }
  ...
  findElementByPont({ x, y }: { x: number; y: number }) {
    ...
    for (const element of sorted) {
      if (
        element.isIntersectRect({ x1: x - 1, y1: y - 1, x2: x + 1, y2: y + 2 })
      ) {
        return element;
      }
    }
  ...
  onMouseMove = (e: MouseEvent) => {
    ...
    if (element) {
      ...
      } else if (this.action.name === "hover") {
        this.action = null;
    ...
    if (this.action) {
      ...
      if (this.action.name === "hover" && this.action.element) {
        this.action.element.setHover(true);
      }
    }
  };
